...
\brief Implementation of class RenderTexture. A multi-format render to texture wrapper.
*/
...
void _wglGetLastError()
{
    ...
    default:
        ...
        log_write(LOG_LEVEL_ERROR, "RenderTexture Win32 Error %d: %s\n", err, lpMsgBuf);
        LocalFree( lpMsgBuf );
        break;
    }
    SetLastError(0);
...
void PrintExtensionError( char* strMsg, ... )
{
    ...
    log_write(LOG_LEVEL_ERROR, strMsg);
}
                                ...
                                bool copyContext        /* = false */)
{
    ...
    log_write(LOG_LEVEL_INFO, "*RT[%dx%d]=(%d,%d,%d,%d)",
        _iWidth, _iHeight, 
        _iNumColorBits[0], _iNumColorBits[1], 
        _iNumColorBits[2], _iNumColorBits[3]);
    if (_iNumDepthBits) log_write_nd(LOG_LEVEL_INFO, " depth=%d", _iNumDepthBits);
    if (_iNumStencilBits) log_write_nd(LOG_LEVEL_INFO, " stencil=%d", _iNumStencilBits);
    if (_bDoubleBuffered) log_write_nd(LOG_LEVEL_INFO, " double buffered");
    long pb_size = _iWidth*_iHeight*(_iNumColorBits[0]+_iNumColorBits[1]+_iNumColorBits[2]+_iNumColorBits[3]);
    if (_bDoubleBuffered) pb_size *= 2;;
    pb_size += _iWidth*_iHeight*_iNumDepthBits;
    pb_size += _iWidth*_iHeight*_iNumStencilBits;
    log_write_nd(LOG_LEVEL_INFO, " mem=%.2fM\n", pb_size / (8*1024.*1024.));
#endif
...
bool RenderTexture::Reset(const char *strMode, ...)
{
    ...
    {
        log_write(LOG_LEVEL_ERROR, "RenderTexture::Reset(): failed to invalidate.\n");
        return false;
    }
...
{
    if (!_bInitialized) {
        log_write(LOG_LEVEL_ERROR, "RenderTexture::Resize(): must Initialize() first.\n");
        return false;
    }
    ...
    if ( _hPBuffer )
    {
        ...
        log_write(LOG_LEVEL_ERROR, "RenderTexture::Resize(): failed to resize.\n");
        return false;
    }
    _bInitialized = false;
                                     ...
                                     vector<int> &pbAttribs)
{
    ...
    if (!WGLEW_NV_render_depth_texture && _bIsDepthTexture && (RT_RENDER_TO_TEXTURE == _eUpdateMode))
    {
        ...
        log_write(LOG_LEVEL_ERROR, "RenderTexture Warning: No support found for "
...
bool RenderTexture::_InitializeTextures()
{
    ...
    { 
        ...
        //woid glTexImage2D(_iTextureTarget, 0, GL_DEPTH_COMPONENT, _iWidth, _iHeight, 0, GL_DEPTH_COMPONENT, GL_FLOAT, 0);
