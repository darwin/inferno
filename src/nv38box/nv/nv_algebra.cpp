...
{
	nv_scalar norm = sqrtf(x * x + y * y + z * z);
	if (norm > nv_eps)
		norm = nv_one / norm;
	else
		norm = nv_zero;
...
vec2 & scale(vec2& u, const nv_scalar s)
{
    u.x *= s;
    u.y *= s;
    return u;
...
{
    nv_scalar oow;
    nv_scalar divider = v.x * M.a30 + v.y * M.a31 + v.z * M.a32 + M.a33;
...
{
    nv_scalar oow;
    nv_scalar divider = v.x * M.a03 + v.y * M.a13 + v.z * M.a23 + M.a33;
    if (divider < nv_eps && divider > -nv_eps)
        oow = nv_one ;
    else
        oow = nv_one / divider;
...
{
    nv_scalar norm = sqrtf(u.x * u.x + u.y * u.y);
    if (norm > nv_eps)
        norm = nv_one / norm;
    else
        norm = nv_zero;
...
void quat::FromMatrix(const mat3& mat)
{
	...
	if (trace > nv_zero) 
    {
		nv_scalar scale = sqrtf(trace + nv_one);
		w = nv_zero_5 * scale;
...
nv_scalar mat3::norm_one()
{
    nv_scalar sum, max = nv_zero;
